...
def seuilTexteBleu(img):
    ...
    for i in range (img.shape[0]):
        for j in range(img.shape[1]):
            if ( (90 <= img[i,j,0] <= 145) and (105 <= img[i,j,1] <= 150) and (137 <= img[i,j,2] <= 163)   #BLEU CLAIR
                or (6 <= img[i,j,0] <= 90) and (47 <= img[i,j,1] <= 100) and (130 <= img[i,j,2] <= 150)    #BLEU NORMAL
                or (3 <= img[i,j,0] <= 34) and (10 <= img[i,j,1] <= 66) and (100 <= img[i,j,2] <= 145) ):  #BLEU FONCE
                n[i,j] = 255
...
def seuilTexteBleuClair(img):
    ...
    for i in range (img.shape[0]):
        for j in range(img.shape[1]):
            if ( (90 <= img[i,j,0] <= 145) and (105 <= img[i,j,1] <= 150) and (137 <= img[i,j,2] <= 163) ): # 1.jpg
                n[i,j] = 255
...
def seuilTexteBleuNormal(img):
    ...
    for i in range (img.shape[0]):
        for j in range(img.shape[1]):
            if ( (6 <= img[i,j,0] <= 90) and (47 <= img[i,j,1] <= 100) and (130 <= img[i,j,2] <= 150) ): # 1.jpg
                n[i,j] = 255
...
def seuilTexteBleuFonce(img):
    ...
    for i in range (img.shape[0]):
        for j in range(img.shape[1]):
            if ( (3 <= img[i,j,0] <= 34) and (10 <= img[i,j,1] <= 66) and (100 <= img[i,j,2] <= 145) ): # 1.jpg
                n[i,j] = 255
...
def main():
    ...
    img = pltimg.imread("D:/Dossier_chelou/Users/kelli/Documents/GitHub/Projet-Image-Suicide-Squad-/Images_Train_et_test/Entrainement_(57)/78.jpg")
    new_img = seuilTexteBleu(img)
